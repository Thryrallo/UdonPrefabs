...
namespace Thry.Udon.UI
{
    ...
    public class Clapper : UdonSharpBehaviour
    {
        ...
        ObjectPool.OneObjectPerPlayerPool _effectPool;
        bool hasNetworkedEffect = false;
        bool hasLocalEffect = false;
        [SerializeField, HideInInspector] HandCollider_Left _left;
        [SerializeField, HideInInspector] HandCollider_Right _right;
        int _claps = 0;
        ...
        private void Update()
        {
            ...
            {
                if (action.desktopKey != "" && Input.GetKeyDown(action.desktopKey) && !VRCLib.IsVRCMenuOpen(_vrcUIColliders))
                {
            ...
            {
                if ((string)action[1] != "" && Input.GetKeyDown((string)action[1]) && !VRCLib.IsVRCMenuOpen(_vrcUIColliders))
                {
                    ((UdonBehaviour)action[2]).SendCustomEvent((string)action[3]);
                }
            }
            //Dekstop claps
            if (Input.GetKeyDown(KeyCode.F) && !VRCLib.IsVRCMenuOpen(_vrcUIColliders))
            {
                _Clap();
            }
        }
